...
const Editar = () => {
  ...
  const getClientByID = async () => {
    try {
      ...
      if (!data) {
        setClient(objClient);
        return setNotFoundMessage(
          `Cliente com o ID ${ID} não foi identificado`
        );
      }
      setNotFoundMessage("");
  ...
  const putUpdate = async () => {
    try {
      ...
      filteredData.idade = filteredData.idade
        ? filteredData.idade
        : client.idade;
      filteredData.documento = filteredData.documento
        ? filteredData.documento
        : client.documento;
      filteredData.senha = filteredData.senha
        ? filteredData.senha
        : client.senha;
  ...
  return (
    <>
      <Button variant="primary" className="align-self-end" onClick={handleShow}>
        {" "}
        Editar{" "}
      </Button>
      <Modal show={modalState} onHide={handleClose} centered>
        ...
        <Modal.Body>
          ...
          {notFoundMessage && <p className="text-danger">{notFoundMessage}</p>}
        </Modal.Body>
        <Modal.Footer>
              ...
              return index == 0 ? (
                ...
                >
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setInput({ [Object.keys(client)[index]]: e.target.value })
                    }
                    defaultValue={value ? value : ""}
                    type={
                      Object.keys(client)[index] == "senha"
                        ? "password"
                        : "text"
                    }
                  />
          ...
          {updateMessageResult && (
            <p className={errorUpdate ? `text-danger` : "text-success"}>
              {updateMessageResult}
            </p>
          )}
          <Button variant="primary" onClick={putUpdate}>
            {" "}
            Atualizar{" "}
          </Button>
        </Modal.Footer>
